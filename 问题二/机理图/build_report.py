"""Build captions, equations, provenance and LaTeX includes from figure data."""
from pathlib import Path
import json

HERE = Path(__file__).resolve().parent
FIGURES = [
    ('图1_epsilon约束与四目标条件筛选', 'ε约束选取：四目标条件筛选'),
    ('图2_自适应epsilon阈值与真实求解状态', '自适应ε：阈值平面、试探与状态'),
    ('图3_ALNS破坏修复与自适应算子', 'ALNS：释放、修复、概率和目标变化'),
    ('图4_覆盖工作量松弛域与分数解', '松弛下界：投影几何与分数路线'),
    ('图5_同条件下界与最坏遗憾区间', '下界核验：同条件界差与偏好遗憾'),
]
CAPTIONS = [
    '最终档案中14个已核验方案的能耗—最晚返场时间投影。迟到上限由30 s收紧至0 s时，符合条件的方案由5个减少至3个，档案内最早返场方案保持不变；进一步将架次上限收紧为20后，仅余1个方案。蓝色背景仅表示能耗上限，标记同时考虑架次和迟到条件。结果为有限档案筛选，不构成ε子问题全局最优认证。',
    '旧558条候选工作库上的自适应ε阈值构造与实际求解状态。档案范围生成架次减一、能耗中点、迟到中点及零迟到四种试探；对应状态来自各自任务日志。第二轮阈值重复而跳过，本次有限试探不代表完整目标空间枚举。',
    '旧候选库上一次15步ALNS运行及第10次局部修复。释放3架次后，局部解包含2条沿用路线和4条新增路线；其余17架次的资源与时刻固定。下排显示本次选择前的算子概率和按接受事件重建的当前解目标轨迹。架次与能耗增加，同时完成时间和迟到降低。',
    '最终28216条候选库的覆盖—机型工作量松弛在能耗与辅助工期显示窗内的投影，以及14个已知可行方案投影的凸包。边界通过原LP支持函数逐边数值核验；点状线表示人工显示窗边界。斜线凸包内部和右图中的分数路线均不代表可执行调度。该下界仅适用于相同候选库和物理假设。',
    '相同最终候选库下五种约束情景的时间松弛下界与对应档案最早可行上界，以及最终方案在12个冻结偏好极点上的遗憾上下界。所有比较保持候选库、评价尺度和情景条件一致。区间表示松弛下界至已知可行上界的差距，不提供完整原问题全局最优认证。',
]


def main():
    d = json.loads((HERE/'mechanism_data.json').read_text(encoding='utf-8'))
    sections = [r'''# 问题二：求解空间与算法过程机理图
...
绘图显示窗为 $58\le E\le72$ kWh、$85\le\widehat C\le160$ min。上边界、右边界等人工裁切线采用点状线。程序以二维支持函数LP逐边细化投影，共 @VERTICES@ 个顶点、@SUPPORT@ 次不同方向支持查询，归一化支持残差不超过 $2\times10^{-8}$。这些是数值容差内的边界核验，不是有理数精确证书。

最小工期松弛值为 **@LP_C@ min**，其解含 **@FRACTIONALS@ 条分数路线**。右图列出其中9条；相同“机型·服务区”可能对应不同箱子组合，其完整候选ID保存在[松弛分数路线.csv](松弛分数路线.csv)。所有14个已知方案点均通过松弛投影包含性核验。
''', r'''
### 同条件时间上下界
...
差距采用 $(UB-LB)/UB$。这里的 $UB$ 是相同情景下14档案中的最早可行工期；相同有限库联合模型的最优工期位于该上下界之间，但区间内的具体值尚未求精。无附加上限时的档案最早方案为23架次；最后一行的上界对应最终综合代表方案的 @FINAL_C@ min，两者优化含义不同。该代表方案为22架次，不作为N≤20或N≤21行的可行上界。
...
当前最终方案本身属于计算 $UB_h$ 的档案，故图中的各差值非负。最终最坏遗憾区间为 **[@REGRET_L@, @REGRET_U@]**；两个最大值不要求来自同一偏好极点。图册复算了可行基准上界与遗憾差值，时间LP下界另行独立重解；偏好LP下界来自带求解状态的最终基准文件。
...
''']
    ...
    filters='\n'.join(f'|({chr(97+i)})|N≤{r["cap"]["N"]}，E≤{r["cap"]["E"]:g}，L≤{r["cap"]["L"]:g}|{len(r["eligible"])}|PF{r["selected"]+1:04d}|{d["archive"][r["selected"]]["objective"][2]/60:.6f}|' for i,r in enumerate(d['epsilon_filters']))
    states=['旧库内已证不可行','限时有解，未获最优认证','限时有解，未获最优认证','限时未获可行解，不能判不可行']
    eps='\n'.join(f'|{i+1}|{r["cap"]["N"]}|{r["cap"]["E"]:.6f}|{r["cap"]["L"]:.6f}|{states[i]}|' for i,r in enumerate(d['historical_epsilon']['records']))
    alns=[]
    for key,label in [('before','释放前'),('after','第10次修复并接受后')]:
        f=d['alns'][key]['objective'];alns.append(f'|{label}|{f[0]}|{f[1]:.6f}|{f[2]/60:.6f}|{f[3]:.6f}|')
    bound_names=['无附加N/E上限','N≤20','N≤21','N≤22','N≤22且E≤66 kWh']
    bounds='\n'.join(f'|{name}|{r["lower_min"]:.6f}|{r["upper_min"]:.6f}|{r["gap_pct"]:.3f}|' for name,r in zip(bound_names,d['bounds']))
    values={'@FILTER_TABLE@':filters,'@EPS_TABLE@':eps,'@ALNS_TABLE@':'\n'.join(alns),'@BOUNDS_TABLE@':bounds,
        '@VERTICES@':str(len(d['projection']['vertices'])),'@SUPPORT@':str(d['projection']['support_calls']),
        '@LP_C@':f'{d["projection"]["min_c_point"][1]:.6f}','@FRACTIONALS@':str(d['projection']['fractional_count']),
        '@FINAL_C@':f'{d["final_objective"][2]/60:.6f}','@REGRET_L@':f'{d["final_regret"][0]:.9f}','@REGRET_U@':f'{d["final_regret"][1]:.9f}'}
    for (stem,title),caption,detail in zip(FIGURES,CAPTIONS,details):
        sections.append(f'\n## {title}\n\n![{title}]({stem}.png)\n\n**图注：** {caption}\n\n[矢量PDF]({stem}.pdf) · [可编辑SVG]({stem}.svg)\n'+detail)
    sections.append(r'''
## 复现与核验
...
''')
    labels={'epsilon_thresholds_recomputed':'四组ε阈值与日志一致','operator_probabilities_recomputed':'15次算子概率和权重更新一致','released_box_conservation':'ALNS释放与修复的箱子集合守恒','fixed_tasks_unchanged':'17条固定任务的资源和时刻一致','lp_bounds_recomputed':'五种情景时间LP下界与原核查一致','projection_edge_certified':'LP投影各边支持残差在数值容差内','known_points_inside_projection':'14个档案点均处于LP投影内','regret_recomputed':'最终偏好上界与遗憾上界复算一致'}
    values['@CHECKS@']='\n'.join(f'- {labels[k]}：{"通过" if v else "未通过"}。' for k,v in d['validations'].items())
    report='\n'.join(sections)
    for key,val in values.items():report=report.replace(key,val)
    (HERE/'机理图说明与核验.md').write_text(report,encoding='utf-8')
    includes=['% 使用 XeLaTeX + ctex + graphicx；假定本文文件位于项目根目录。']
    for i,((stem,title),caption) in enumerate(zip(FIGURES,CAPTIONS),1):
        includes.append('\n'.join([r'\begin{figure}[htbp]',r'\centering',
            r'\includegraphics[width=\textwidth]{问题二/机理图/'+stem+'.pdf}',
            r'\caption{'+caption+'}',r'\label{fig:q2-mechanism-'+str(i)+'}',r'\end{figure}']))
    (HERE/'latex_includes.tex').write_text('\n\n'.join(includes)+'\n',encoding='utf-8')
    plan='# 问题二机理图计划（已执行）\n\n用户指定内容：ε约束选取、自适应、ALNS与松弛下界。参考问题一同名机理图的排版。\n\n|图|内容|数据范围|实现|\n|---|---|---|---|\n'
    scopes=['最终14方案档案筛选','旧558库真实ε日志','旧558库15步ALNS日志','最终28216库覆盖工作量LP','最终同库时间界与12偏好极点']
    for i,((stem,title),scope) in enumerate(zip(FIGURES,scopes),1):plan+=f'|{i}|{title}|{scope}|Python / Matplotlib，PNG + SVG + PDF|\n'
    (HERE/'FIGURE_PLAN.md').write_text(plan,encoding='utf-8')
    print('Report, figure plan and LaTeX includes written.')
