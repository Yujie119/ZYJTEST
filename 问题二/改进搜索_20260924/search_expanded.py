...
def worker(task):
    idx,steps,seconds,initials,*options=task
    custom=options[0] if options else {}
    rng=random.Random(20260924+idx)
    data=load_data()
    ...
    base_caps=custom.get('cap',{})
    def in_scope(p):
        return all(p['objective']['NECL'.index(k)]<=v+1e-7 for k,v in base_caps.items())
    archive=[]
    for p in inherited:
        validate(data,p)
        if in_scope(p):archive_add(archive,p)
    ...
    for j in range(initials):
        ...
        if validate(data,p) and in_scope(p):archive_add(archive,p)
    ...
    for step in range(steps):
        ...
        cap=dict(base_caps)
        if not forced:
            cap['C']=min(cap.get('C',math.inf),float(coeff@current['objective'])/coeff[2]+1e-5)
