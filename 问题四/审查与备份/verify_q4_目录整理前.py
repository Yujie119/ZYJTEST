...
ROOT=Path(__file__).resolve().parent;OUT=ROOT/'结果'
...
def verify():
    ...
    result=read(OUT/'最终代表方案.json');all_saved=read(OUT/'完整分区档案.json')
    exact_csv=csvread(OUT/'分区枚举明细.csv');expected_rows={}
    ...
    witness=csvread(OUT/'组内资源分配见证.csv');taskrows=csvread(OUT/'分组任务明细.csv')
    certs=read(OUT/'资源峰值证书.json');boxrows=csvread(OUT/'逐箱继承核验.csv')
    for G in [2,3]:
    ...
    for log in read(OUT/'MILP与LP核验.json'):
    ...
    for alt in read(OUT/'备选快照比较.json'):
    ...
    for rec in read(OUT/'偏好与类别权重敏感性.json'):
    ...
    original=load_workbook(ROOT/'输入快照/原始附件/结果提交模板.xlsx');export=load_workbook(ROOT/'问题四_结果提交.xlsx')
...
if __name__=='__main__':
    ...
    (OUT/'核验报告.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
    (OUT/'逐项核验记录.json').write_text(json.dumps(checks,ensure_ascii=False,indent=2),encoding='utf-8')
