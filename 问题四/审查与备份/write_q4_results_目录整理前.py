...
PAPER=HERE.parent/'正文/正文.md';OUT=HERE/'结果'
...
summary+='主方案与3个备选完成相同口径比较；另3个备选有约10^-12 s的原资源交接误差，原值保留并标明精度待统一。\n\n文件入口：\n\n- `问题四_结果提交.xlsx`：官方模板Q4页，仅两个选定代表的5个组。\n- `第7章_结果摘录.md`：已经回填正文的公式、结果表与分析。\n- `结果/分区枚举明细.csv`、`非支配档案.csv`：完整划分和前沿。\n- `结果/组内资源分配见证.csv`、`资源峰值证书.json`：最低数量的可执行资源匹配。\n- `结果/MILP与LP核验.json`、`核验报告.json`：求解及独立核验记录。\n'
