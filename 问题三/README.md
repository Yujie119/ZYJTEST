...
> 专家换电补充及长时搜索：共享电池初始在O01；无人机不能携带备用电池，但允许在实际有电池的地点换装。现数值搜索仅采用O01换电可行策略，尚未覆盖异地作业规则未定的完整事件模型。见[专家补充](审查资料/专家换电补充_建模影响.md)、[本轮已完成搜索与改进](搜索精修/扩展固定结构精修_v1/扩展搜索与专家补充.md)和[两小时后台运行入口](扩展搜索后台/当前运行.json)。后台实际进度以运行目录的`后台状态.json`为准；由独立gpt-5.6-luna任务每10分钟轮询，主对话无需持续运行。

> 2026-09-24 审计更新：旧方案本轮原始算术452项、原DEM核验803项通过，但已找到严格支配见证，故不称其已证最优。旧ALNS还存在模式副本、区间合并和参数/缓存链缺口。请先读[最新复核结论](问题三_从题意到实现的复核结论.md)。下列历史正式结果保留，新增直接联合入口不覆盖它们。

本轮新增 `solve_q3_global.py` / `q3_direct_milp.py`：在冻结有限库内保持1445个路线实例全部可选，迭代补通信与资源约束，支持N/E/C/L和ε上限；ALNS保留作对照来源。小实例43项检查通过。早期120.144 s试跑及后续4×300 s搜索均未得新的全库完整可行解；另有8个结构时序精修通过核验，并产生严格改进。两小时搜索采用补充134个单站模式后的独立新库（1426记录、1424模式、1596实例），旧库下界不能直接转移。`全局直接求解/`保存历史状态，`扩展搜索后台/`保存长时运行，文件名“global”不表示原题连续全空间已证优。

```powershell
& 'E:\tool\anaconda3\python.exe' -X utf8 '问题三\solve_q3_global.py' --seconds 120 --round-seconds 25 --mode C
& 'E:\tool\anaconda3\python.exe' -X utf8 '问题三\审查资料\audit_source_and_result.py'
& 'E:\tool\anaconda3\python.exe' -X utf8 '问题三\审查资料\test_direct_joint.py'
```

上述新增入口只向审查/直接求解目录输出；下方旧全流水线会重写正式结果，注意其历史用途。
